...
export default function ThreeBackground() {
  ...
  useEffect(() => {
    ...
    const ringColorsArr = new Float32Array(ringParticleCount * 3);
    ...
    for (let i = 0; i < ringParticleCount; i++) {
      ...
      ringColorsArr[i * 3] = mixColor.r;
      ringColorsArr[i * 3 + 1] = mixColor.g;
      ringColorsArr[i * 3 + 2] = mixColor.b;
    }

    const ringGeo = new THREE.BufferGeometry();
    ringGeo.setAttribute('position', new THREE.BufferAttribute(ringPositions, 3));
    ringGeo.setAttribute('color', new THREE.BufferAttribute(ringColorsArr, 3));
    ...
    for (let i = 0; i < particleCount && lineCount < maxLines; i++) {
      for (let j = i + 1; j < particleCount && lineCount < maxLines; j++) {
        ...
        if (dist < maxDist && dist > 2) {
          ...
          for (let k = 0; k < 2; k++) {
            lineColorsArr.push(lineColor.r * alpha, lineColor.g * alpha, lineColor.b * alpha);
          }
          lineCount++;
        }
      }
    ...
    lineGeo.setAttribute('color', new THREE.BufferAttribute(new Float32Array(lineColorsArr), 3));
    const lineMatObj = new THREE.LineBasicMaterial({
      vertexColors: true,
      transparent: true,
      opacity: isDark ? 0.1 : 0.05,
    });
    const lineNetwork = new THREE.LineSegments(lineGeo, lineMatObj);
    ...
    const animate = () => {
      ...
      // Ring rotation
      ringPoints.rotation.x = Math.sin(t * 0.2) * 0.5;
      ringPoints.rotation.y = t * 0.15;
      ringPoints.rotation.z = Math.cos(t * 0.1) * 0.3;
